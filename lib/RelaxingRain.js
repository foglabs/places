...
class RelaxingRain extends Place {
 ...
 console.log( 'its ', charqfgf20fm3nd2.mesh.id )
    ...
    let p,pc,pg,pm,pp, pos
    for(var i=0; i<8; i++){
      pg = new THREE.SphereGeometry(0.5+Math.random(), 32, 32)

      pc = util.rgbToHex( Math.floor(Math.random()*255),Math.floor(Math.random()*255),Math.floor(Math.random()*255) )
      pm = new THREE.MeshBasicMaterial({color: pc})

      p = new THREE.Mesh(pg,pm)


      pp = util.randomPointAway(64,20)
