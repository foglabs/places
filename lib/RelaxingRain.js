...
class RelaxingRain extends Place {
  constructor(){
    ...
    // I need a light
    let light = new THREE.SpotLight( 0xffffdd, 0.004, 100 )
    ...
    // char4wd7el71edr.mesh.position.set(0,2.5,-0.3499999999999999)
    ...
    charm6awq5verz.mesh.position.set(0.6500000000000004,1.7,0.3200000000000006)
    ...
    charrjbg8ps445p.mesh.position.set(-0.6700000000000004,1.7,0.3200000000000006)
    ...
    charoxfjg4clrw.mesh.position.set(-0.6600000000000004,1.7,-1.0300000000000002)
    ...
    char4rsqwejs7bc.mesh.position.set(0.6600000000000004,1.7,-1.0300000000000002)
    ...
    char24xq7w9ca9.mesh.position.set(-0.4,1.19,-0.9499999999999999)
    ...
    charqfgf20fm3nd.mesh.position.set(0.2900000000000003,1.2,-0.86)
    ...
    charqfgf20fm3nd2.mesh.position.set(0.0900000000000003,1.2,-0.86)
    ...
    charctw2g1t5rg5.mesh.position.set(-0.4,1.008,-0.9399999999999999)
    ...
    for(let i=0;i<10000;i++) {
      x = util.randomSign() * Math.random() * 100
      y = util.randomSign() * Math.random() * 100 + 300
      z = util.randomSign() * Math.random() * 100
    ...
    for(let p=0; p<128; p++) {
      ...
      cloud.rotationSpeed = 0.003 * Math.random() - 0.0005
      ...
      // osunds
      let tracks = []
      let urls = ["https://fog-places.s3.amazonaws.com/BlueLight/01+-+frek.mp3",
]

      let url
      for(var i=0; i<urls.length; i++){
        url = urls[i]
        tracks.push( new Sound("track"+i, urls[i], 1, false, false) )
      }

      this.musicPlayer = new MusicPlayer(tracks)
    }
    ...
    for(var i=0; i<10; i++){
      ...
      mountainMat = new THREE.MeshPhysicalMaterial( {transparent: false, color: 0xffff00, side: THREE.DoubleSide, displacementMap: selectedMap, displacementScale: 8.2 } )


      this.mountains[i] = new THREE.Mesh( mountainGeo, mountainMat )
      // at least 4 away, random within 0-5, pos or neg
      x = util.randomSign() * (Math.random() * 5 + 10)
      y = 7
      z = util.randomSign() * (Math.random() * 5 + 10)
