...
import { textureLoader, scene, util } from "./main.js"
...
class Demo extends Place {
  constructor(){
    ...
    light.position.set(-0.6600000000000004,2,-1.0300000000000002)
    light.lookAt(0,1.2,-0.3499999999999999)
    ...
    let woodMap = textureLoader.load('/textures/rain-wood-floor.png') 
    woodMap.wrapS = THREE.RepeatWrapping
    woodMap.wrapT = THREE.RepeatWrapping
    woodMap.repeat.set(2,2)
    let woodBump = textureLoader.load('/textures/rain-wood-floorB.png') 
    woodBump.wrapS = THREE.RepeatWrapping
    woodBump.wrapT = THREE.RepeatWrapping
    woodBump.repeat.set(2,2)

    let mountainMap = textureLoader.load("/textures/mountainD.png")

    let mat =  new THREE.MeshPhysicalMaterial({map: woodMap, bumpMap: woodBump, bumpScale: 0.2})

    let characters = {}

    let wall = new JunkPiece(new THREE.BoxGeometry(2, 2, 0.1), new THREE.Box3(new THREE.Vector3(), new THREE.Vector3()), [120,120,120], mat)
    wall.setPosition(0,0.8,2.5)
    wall.pushable = false
    scene.add(wall.mesh)
    characters[wall.mesh.id] = wall

    let wall2 = new JunkPiece(new THREE.BoxGeometry(1, 1, 0.1), new THREE.Box3(new THREE.Vector3(), new THREE.Vector3()), [120,120,120], mat)
    ...
    char23491miehjf.move(0.2,1,-0.5)
    char23491miehjf.mesh.rotation.set(0,0,0)
    scene.add( char23491miehjf.mesh )
    characters[char23491miehjf.mesh.id] = char23491miehjf

    var char6dlf0kvj5bk = new Character( new THREE.BoxGeometry( 0.08000000000000002,0.5499999999999997,0.08000000000000002 ), new THREE.Box3(new THREE.Vector3(), new THREE.Vector3()), [21,116,100], new THREE.MeshPhysicalMaterial( { map: woodMap, bumpMap: woodBump, bumpScale: 0.08, color: '#ffffff', transparent: true, transmission: 0.5, reflectivity: 1, roughness: 0, clearcoat: 1.0, clearcoatRoughness: 0.1 }) )
    char6dlf0kvj5bk.move(0.11999999999999998,1,-0.5)
    ...
    charemk64ptg2g6.move(-0.04000000000000005,1,-0.5)
    ...
    charhqrbj6qyll.move(-0.16000000000000015,1,-0.5)
    ...
    charspsd61t1nrf.move(-0.27000000000000025,1,-0.5)
    ...
    char6d65qt1pvt9.move(-0.00000000000000004,1,-0.5)
    ...
    charpz9evj23her.move(0.36000000000000015,1.2,-0.03)
    ...
    charivk9xnmlmh.move(0.36000000000000015,1.2,-0.5200000000000002)
    ...
    charyqwybqd5wm.move(0.8100000000000005,1.2,-0.26)
    ...
    char8nlhojct64c.move(0.05999999999999992,1.2,-0.29000000000000004)
    ...
    charq9c90nqtfbc.move(0.7000000000000004,1.2,-0.44000000000000017)
    ...
    charpfmmm449km9.move(0.5300000000000002,1.2,-0.6000000000000003)
    ...
    charxw9srji3yj.move(0.6300000000000003,1.2,-0.16999999999999993)
    ...
    let ck = util.k(characters)
    for(var i=0; i<ck.length; i++){
        characters[ck[i]].mesh.castShadow = true
        characters[ck[i]].mesh.receiveShadow = true
    }
    ...
    // da floor
    let floorMap = textureLoader.load('/textures/floor-text.jpg') 
    floorMap.wrapS = THREE.RepeatWrapping
    floorMap.wrapT = THREE.RepeatWrapping

    var floor = new JunkPiece(new THREE.BoxGeometry(5, 0.1, 5), new THREE.Box3(new THREE.Vector3(), new THREE.Vector3()), [255,120,250], new THREE.MeshPhysicalMaterial( { map: floorMap, transparent: true, transmission: 0, reflectivity: 1, roughness: 0.5, clearcoat: 1.0, clearcoatRoughness: 0.1, bumpMap: floorMap, bumpScale: 0.08 }) )
