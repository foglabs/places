...
import { util, gltfLoader, scene } from "./main.js"
...
class Place {
  ...
  loadModel(path, imDone){
    // if we need to, load it
    gltfLoader.load(path, function(gltf){
      let mesh = gltf.scene.children[0]
      
      // save for next itme
      // pass it to where w tried to use it
      imDone(mesh)
    })  
  }
  ...
  isTouchingFloor(char){
    // if i'm touching floor, how far is char's box3 center from...
    // let center = new THREE.Vector3
    // center = char.bbox.getCenter(center)
