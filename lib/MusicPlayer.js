...
class MusicPlayer extends Character {
  ...
  setDisplayText(text){
    // console.log( 'set txt', text )
    this.displayText = text
    this.displayTextIndex = 0
  }

  displayAnimation(){
    if(this.displayTextTimer.time() > 646){
    ...
    if(!this.idle && this.displayIdleTimer.time() > 4000){
      ...
      let text = this.displayDefaultText
      if(this.playing){
        text = "♫ " + this.album.artist + " - " + this.album.title + " - "  + this.album.trackTitles[this.currentTrack] + " ♫ PLAYING "
      }
  ...
  loadAlbum(urls, titles){
    let url, sound

    let tFinish = () => {
      console.log( 'tifnish babgby' )
      this.nextTrack()
    }
    ...
    for(var i=0; i<urls.length; i++){
      ...
      sound = new Sound("track"+i, urls[i], this.volume, false, false, false, null, null, tFinish )
