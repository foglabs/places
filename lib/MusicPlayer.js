...
class MusicPlayer extends Character {
  constructor(album=null){
    ...
    let faciaoffset = new THREE.Vector3(0,-0.02,0.05)
    ...
    let ejectfaciaoffset = new THREE.Vector3(0,0.07,0.05)
    ...
    this.playButton = new Button( this, offset, playgeo, [255,255,0], playmat, pbClick )
    ...
    let ejectoffset = new THREE.Vector3(-0.06,0.07,0.05)
    ...
    let traygeo = new THREE.BoxGeometry( 0.18, 0.02, 0.06 )
    ...
    this.setDisplayText("FOG STEREO QX5       ")
  ...
  openAnimation(){
    console.log( 'opening' )
    if( !util.isWithin(this.tray.mesh.rotation.x, util.radian(120), 0.02)  && (!this.album || !util.isWithin(this.album.mesh.rotation.x, util.radian(120), 0.02) ) ){
  ...
  createDisplay(){
    ...
    let dispBackOffset = new THREE.Vector3(0,0.02,0.052)
    this.displayBack = new Component(this, dispBackOffset, new THREE.PlaneGeometry(0.18,0.04), [221,10,255], new THREE.MeshPhysicalMaterial({transparent: false, clearcoat: 0.8, transmission: 0, bumpMap: wallBump, bumpScale: 0.09}))
    ...
    let dispOffset = new THREE.Vector3(0,0.0175,0.05201)
