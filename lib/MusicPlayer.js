...
class MusicPlayer extends Character {
  constructor(album=null){
    ...
    this.stopButton = new Component( this, stopoffset, stopgeo, [255,0,0], stopmat, () => { if(this.album){ this.stopSong(this.currentTrack) } } )
    ...
    let nextmat = new THREE.MeshStandardMaterial( { map: stereoFrontMap, bumpMap: stereoFrontBump, alphaMap: stereoFrontAlpha, color: "#ff00ff" })
    let nextoffset = new THREE.Vector3(0.06,-0.02,0.05)
    this.nextButton = new Component( this, nextoffset, nextgeo, [255,0,255], nextmat, () => { if(this.album){ this.nextTrack() } } )
    ...
    let prevmat = new THREE.MeshStandardMaterial( { map: stereoFrontMap, bumpMap: stereoFrontBump, alphaMap: stereoFrontAlpha, color: "#ffff00" })
    ...
    let traygeo = new THREE.BoxGeometry( 0.18, 0.01, 0.03 )
    let traymat = new THREE.MeshBasicMaterial( { color: "#ffff00" })
    ...
    let ejectmat = new THREE.MeshStandardMaterial( { map: stereoFrontMap, bumpMap: stereoFrontBump, alphaMap: stereoFrontAlpha, color: "#ffff00" })
  ...
  openAnimation(){
    console.log( 'opening' )
    if(this.tray.mesh.rotation.x < util.radian(120)){
      this.tray.mesh.rotation.x += 0.01
    } else {
      this.state = OPEN
    }
  }
