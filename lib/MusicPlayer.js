...
class MusicPlayer extends Character {
  constructor(album=null){
    ...
    let stopmat = new THREE.MeshStandardMaterial( { map: stereoMap,map: stereoMap, bumpMap: stereoBump, color: "#ff0000" })
    let stopoffset = new THREE.Vector3(-0.02,-0.02,0.05)
    this.stopButton = new Component( this, stopoffset, stopgeo, [255,0,0], stopmat, () => { this.stopSong(this.currentTrack) } )
    ...
    let playmat = new THREE.MeshStandardMaterial( { map: stereoMap,map: stereoMap, bumpMap: stereoBump, color: "#00ff00" })
    ...
    let nextmat = new THREE.MeshStandardMaterial( { map: stereoMap, bumpMap: stereoBump, alphaMap: stereoAlpha, color: "#ff00ff" })
    let nextoffset = new THREE.Vector3(0.06,-0.02,0.05)
    this.nextButton = new Component( this, nextoffset, nextgeo, [255,0,255], nextmat, () => { this.nextTrack() } )
    ...
    let prevmat = new THREE.MeshStandardMaterial( { map: stereoMap, bumpMap: stereoBump, alphaMap: stereoAlpha, color: "#ffff00" })
    let prevoffset = new THREE.Vector3(-0.06,-0.02,0.05)
    this.prevButton = new Component( this, prevoffset, prevgeo, [255,0,255], prevmat, () => { this.prevTrack() } )
    ...
    let traygeo = new THREE.BoxGeometry( 0.12, 0.01, 0.03 )
    let traymat = new THREE.MeshStandardMaterial( { map: stereoFrontMap, bumpMap: stereoFrontBump, alphaMap: stereoFrontAlpha, color: "#ffff00" })
    let trayoffset = new THREE.Vector3(0,0.102,0.2)
    this.tray = new Component( this, trayoffset, traygeo, [255,255,255], traymat )
    scene.add(this.tray.mesh)
    this.components.push( this.tray )

    let ejectgeo = new THREE.BoxGeometry( 0.03, 0.02, 0.04 )
    let ejectmat = new THREE.MeshStandardMaterial( { map: stereoMap, bumpMap: stereoBump, alphaMap: stereoAlpha, color: "#ffff00" })
    let ejectoffset = new THREE.Vector3(-0.06,0.08,0.05)
    this.ejectButton = new Component( this, ejectoffset, ejectgeo, [255,255,0], ejectmat, () => { this.eject() } )
  ...
  insert(album){
    ...
    this.album = album
    this.loadAlbum( this.album.trackUrls )
    this.setDisplayText(this.album.title)
  }

  ejectAnimation(){
    if(this.tray.mesh.rotation.z < util.radian(90)){
      this.tray.mesh.rotation.z += 0.01
    } else {
      this.state = IDLE
    }
  }
  ...
  createDisplay(){
    ...
    this.displayText = "FOG STEREO SYSTEM OPERATING AT 4.0080 MHZ OVER AIRWAVE AND THE WIRE "
