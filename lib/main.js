    ...
    function init() {
      ...
      // cam away
      camera = new THREE.PerspectiveCamera( 100, window.innerWidth / window.innerHeight, 0.01, 240 )
      ...
      for(var i=0; i<charKeys.length; i++){
        ...
        if ( userData.isSelecting === true ) {
          if(!pointer.grabbedId && char.grabbable && pointer.grabBox.bbox.intersectsBox(char.bbox)){
            ...
            // if it came to rest somewhere, turn gravity back on so it falls when we drop it
            char.stopped = false
          }
        }
