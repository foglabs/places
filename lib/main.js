    ...
    function init() {
      ...
      camGroup.position.set(-0.4,0,-0.39)
    ...
    function handleController( controller ) {
      ...
      if(pointer.grabbedId) {
        ...
        let grabbed = place.characters[ pointer.grabbedId ]
        grabbed.move(conPos.x, conPos.y, conPos.z)
        ...
        if(grabbed.rotatable){
          ...
          objRot.z *= 1.3
          objRot.x *= 1.3
          grabbed.mesh.setRotationFromEuler( objRot )
          
          let thiscomp
          if(grabbed.components.length > 0){
            for(var i=0; i<grabbed.components.length; i++){
              thiscomp = grabbed.components[i]
              // also rotate components when carrying object
              thiscomp.mesh.setRotationFromEuler( objRot )
            }
          }  
        }
      ...
      for(var i=0; i<charKeys.length; i++){
        ...
        if(char.components.length > 0){
          for(var z=0; z<char.components.length; z++){
            ...
            // default
            if(char.components[z].clickable && char.components[z].clickTimer.time() > 80){
              char.components[z].clicked = false
            }
          }
        }
        ...
        // dont ju8st click on a component if we're already grabbing something
        if ( !pointer.grabbedId && userData.isSelecting === true ) {
