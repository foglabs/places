...
class Character {
  constructor(geo, bbox, base_color, mat=null, clickAction=null, dropAction=null, modelMesh=null, customBboxScale=null){
    this.accx = 0
    this.accy = 0
    this.accz = 0
    ...
    if(modelMesh){
      // passed in mesh, skip making material etc
      this.mesh = modelMesh
    } else {
      // make the mesh ourself

      if(!mat){
        // if we didnt explicitly pass the material in
    ...
    // reflect da cola
    this.mesh.material.color.setRGB(base_color[0], base_color[1], base_color[2])

    this.bbox = bbox
    this.customBboxScale = customBboxScale
    ...
    // 'grabbics'
    this.grabbable = true
    // these correspond to functions that trigger on click/drop
  ...
  arrivedAtScale(){
    return this.mesh.scale.x == this.scalex && this.mesh.scale.y == this.scaley && this.mesh.scale.z == this.scalez
  }
  ...
  handleScale(){
    ...
    setx = util.incTowards(x, this.scalex, 0.008)
    sety = util.incTowards(y, this.scaley, 0.008)
    setz = util.incTowards(z, this.scalez, 0.008)

    // tfw fuck
    if(util.isWithin(x, this.scalex, 0.02)){
      setx = this.scalex
    }
    if(util.isWithin(y, this.scaley, 0.02)){
      sety = this.scaley
    }
    if(util.isWithin(z, this.scalez, 0.02)){
      setz = this.scalez
    }
    this.mesh.scale.set(setx,sety,setz)
  }
  ...
  handleMovement(){
    ...
    if(this.rotatable && !this.arrivedAtRotation()){
      // rotate towards rotx rot y
      this.handleRotation()
    }

    if(this.scalable && !this.arrivedAtScale()){
      // if it aint scaled, scale it
      this.handleScale()
    }
  ...
  gravity(){
    ...
    if(!place.isTouchingFloor(this)){
      ...
      this.hitFloor = true
      if(newHitFloor){
        // actually pop up slightly so we dont flal through
       newAcc = 0.008
      } else {
       newAcc = 0
      }
    ...
    if(this.accy != 0 && this.components.length>0){
      // bring components with thier parent
      let thisComp
      for(var i=0; i<this.components.length; i++){
        thisComp = this.components[i]
        thisComp.mesh.position.set( this.mesh.position.x + thisComp.offset.x,this.mesh.position.y + thisComp.offset.y,this.mesh.position.z + thisComp.offset.z )

        thisComp.mesh.rotation.x = this.mesh.rotation.x
        thisComp.mesh.rotation.y = this.mesh.rotation.y
        thisComp.mesh.rotation.z = this.mesh.rotation.z
      }
    }
  ...
  animation(){
    ...
    if(this.customBboxScale){
      ...
      let min,max
      min = new THREE.Vector3(this.bbox.min.x,this.bbox.min.y,this.bbox.min.z)
      max = new THREE.Vector3(this.bbox.max.x*this.customBboxScale.x,this.bbox.max.y*this.customBboxScale.y,this.bbox.max.z*this.customBboxScale.z)
