...
class Character {
  ...
  addRandomRotation(){
    if( Math.abs(this.accx) > 0 ){
      this.rotx = Math.random() * util.radian( 180 ) - util.radian( 90 )
    }
    if( Math.abs(this.accy) > 0 ){
      this.roty = Math.random() * util.radian( 180 ) - util.radian( 90 )
    }
    if( Math.abs(this.accz) > 0 ){
      this.rotz = Math.random() * util.radian( 180 ) - util.radian( 90 )
    }        
  }
  ...
  handleRotation(){
    ...
    // close enough!
    let snapx,snapy,snapz
    if(util.isWithin(this.mesh.rotation.x, this.rotx, 0.1)){
      snapx = this.rotx
    }
    if(util.isWithin(this.mesh.rotation.y, this.roty, 0.1)){
      snapy = this.roty
    }
    if(util.isWithin(this.mesh.rotation.z, this.rotz, 0.1)){
      snapz = this.rotz
    }
    // rotate smoothly towards some rotation we've hcosen elsewhere
    if(!snapx){

      let amtx = Math.random() * 0.012
      snapx = util.rotateToward(this.mesh.rotation.x, this.rotx, amtx)
    }    
    if(!snapy){

      let amty = Math.random() * 0.012
      snapy = util.rotateToward(this.mesh.rotation.y, this.roty, amty)
    }
    if(!snapz){

      let amtz = Math.random() * 0.012
      snapz = util.rotateToward(this.mesh.rotation.z, this.rotz, amtz)
    }

    this.mesh.rotation.x = snapx
    this.mesh.rotation.y = snapy
    this.mesh.rotation.z = snapz
  }
