...
class Character {
  ...
  handleRotation(){
    ...
    if(!setx){
      ...
      let amtx = Math.random() * 0.01 + 0.005
      setx = util.rotateToward(x, this.rotx, amtx)
    }    
    if(!sety){

      let amty = Math.random() * 0.01 + 0.005
      sety = util.rotateToward(y, this.roty, amty)
    }
    if(!setz){

      let amtz = Math.random() * 0.01 + 0.005
      setz = util.rotateToward(z, this.rotz, amtz)
    }

    // tfw fuck
    if(util.isWithin(x, this.rotx, 0.02)){
      setx = this.rotx
    }
    if(util.isWithin(y, this.roty, 0.02)){
      sety = this.roty
    }
    if(util.isWithin(z, this.rotz, 0.02)){
      setz = this.rotz
    }
