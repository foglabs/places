...
class Character {
  ...
  handleRotation(){
    ...
    // close enough!
    let snapx,snapy,snapz
    if(util.isWithin(this.mesh.rotation.x, this.rotx, 0.001)){
      snapx = this.rotx
    }
    if(util.isWithin(this.mesh.rotation.y, this.roty, 0.001)){
      snapy = this.roty
    }
    if(util.isWithin(this.mesh.rotation.z, this.rotz, 0.001)){
      snapz = this.rotz
    }
    // rotate smoothly towards some rotation we've hcosen elsewhere
    if(!snapx){

      let amtx = Math.random() * 0.005
      snapx = util.rotateToward(this.mesh.rotation.x, this.rotx, amtx)
    }    
    if(!snapy){

      let amty = Math.random() * 0.005
      snapy = util.rotateToward(this.mesh.rotation.y, this.roty, amty)
    }
    if(!snapz){

      let amtz = Math.random() * 0.005
