...
class Character {
  ...
  arrivedAtPoint(){
    return util.isWithin(this.mesh.position.x, this.routePoints[0][0], 0.01) && util.isWithin(this.mesh.position.y, this.routePoints[0][1], 0.01) && util.isWithin(this.mesh.position.z, this.routePoints[0][2], 0.01)
  }
  ...
  handleRoutePoints(){
    ...
    if(this.action == MOVE){
      // console.log( 'moving' )
      this.moveTowardsPoint( this.routePoints[0][0], this.routePoints[0][1], this.routePoints[0][2], 1 )
      ...
      if(this.waitTimer.time() > 4000){
        this.waitTimer.reset()

        this.action = MOVE
        this.currentRouteIndex += 1
        // console.log( 'next!', this.currentRouteIndex )
  ...
  //   let dir = new THREE.Vector3( this.accx, this.accy, this.accz )
  ...
  moveTowardsPoint(destx, desty, destz, speedFactor=1){
    ...
    let speed
    if(xdiff < 0.2 && ydiff < 0.2 && zdiff < 0.2){
      // 2x speed once were close.. somehow helps us get there
      speed = 0.08 * speedFactor
    } else {
