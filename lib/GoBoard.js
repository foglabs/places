...
class GoBoard extends Character {
          ...
          if(this.stones[row][column].goStoneColor == searchColor ){
            ...
            if( !foundInCurrentGroup ){
              ...
              if( this.checkStone(row, column-1, searchColor) ){
                console.log( 'left had truth' )
                result = true
              }

              // up
              if( this.checkStone(row+1, column, searchColor) ){
                console.log( 'up had truth' )
                result = true
              }

              // right
              if( this.checkStone(row, column+1, searchColor) ){
                console.log( 'right had truth' )
                result = true
              }

              // down
              if( this.checkStone(row-1, column, searchColor) ){
                console.log( 'down had truth' )
                result = true
              }
  ...
  computeBoard(colorToRemove){
    ...
    for(var i=0; i<this.boardLength; i++){
      for(var x=0; x<this.boardLength; x++){
        ...
        // console.log( 'was it in stonesToRemove', stonesToRemove.some( (stone) => {stone[0] == x && stone[1] == i }) )
          ...
          if( !stonesToRemove.some( (stone) => {stone[0] == i && stone[1] == x }) ){
            ...
            if(!foundLiberty){
              ...
              if(this.currentGroup.length > 0){
                ...
                console.log( 'I got stonestoremove', stonesToRemove )
              } else {
                console.log( 'removing', i,x )
                // remove the og stone, didnt find a group
                console.log( 'I chose to remove the 1 stone' )
  ...
  //   // above
  //   if( (row-1 < 0) || (this.stones[row-1][column] && this.stones[row-1][column].goStoneColor != this.stones[row][column].goStoneColor) ){
  //     liberties -= 1
  //   }

  //   // right
  //   if( (column+1 == this.boardLength) || (this.stones[row][column+1] && this.stones[row][column+1].goStoneColor != this.stones[row][column].goStoneColor)){
  //     liberties -= 1
  //   }

  //   // below
  //   if( (row+1 == this.boardLength) || (this.stones[row+1][column] && this.stones[row+1][column].goStoneColor != this.stones[row][column].goStoneColor) ){
  //     liberties -= 1
  //   }

  //   // left
  //   if( (column-1 < 0) || (this.stones[row][column-1] && this.stones[row][column-1].goStoneColor != this.stones[row][column].goStoneColor) ){
  //     liberties -= 1
  //   }
